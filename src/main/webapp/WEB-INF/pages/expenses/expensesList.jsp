                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless" id="dataTable" style="width: 100%;" cellspacing="0">
                            ...
                            <c:forEach items="${expensesList}" var="el">
                                <c:if test="${el.active eq 1}">
                                    <tr role="row" class="odd">
                                        <td>${el.id_expenses}</td>
                                        <td>${el.date_insert}</td>
                                        <td>${el.name}</td>
                                        <td>${el.expensesType.expenses_type}</td>
                                        <td>${el.amount}</td>
                                        <td>
                                            <div class="btn-group" style="float: right">
                                                <a class="btn au-btn--blue"
                                                   href="dowloadExpenses/${el.id_expenses}">
                                                    <i class="fas fa-download"></i>Dowload
                                                </a>
                                                <a class="btn btn-primary"
                                                   href="editExpenses/${el.id_expenses}">
                                                    <i class="fas fa-pencil-alt"></i>Edit
                                                </a>
                                                <a class="btn btn-danger"
                                                   href="javascript: deleteExpenses('${el.id_expenses}','${el.name}');">
